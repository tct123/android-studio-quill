...
//	}
	...
	public boolean addTagToCloud(Tag tag) {
		...
		for (int i=0; i<N; i++) {
			...
			t.moveToOrigin();
			t.positionSlideInFromInfinty((float)Math.cos(phi), (float)Math.sin(phi));
			float r =(float)Math.sqrt(t.x*t.x+t.y*t.y);
			if (i==0) 
				r_min = r;
			else if (r<r_min) {
				r_min = r;
		...
		t.moveToOrigin();
		t.positionSlideInFromInfinty((float)Math.cos(phi_min), (float)Math.sin(phi_min));
		tagLayout.add(t);
		return true;
	}
